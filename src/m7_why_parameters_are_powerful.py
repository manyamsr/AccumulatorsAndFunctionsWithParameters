...
def better_draw_circles(radii_multiplier):
    window = rg.RoseWindow(800, 800)

    center = rg.Point(400, 400)
    for k in range(21):
        circle = rg.Circle(center, radii_multiplier * k)
        circle.attach_to(window)
        window.render(0.05)  # Pauses for 0.05 seconds after rendering.
...
def run_test_even_better_draw_circles():
    ...
    print('Testing  draw_circles:  See graphics window')
    print('--------------------------------------------------')
    even_better_draw_circles(3, 30, rg.Point(200, 200))
    even_better_draw_circles(6, 15, rg.Point(300, 300))
    even_better_draw_circles(9, 5, rg.Point(125, 125))


def even_better_draw_circles(radii_multiplier, num_circles, center_location):
    window = rg.RoseWindow(800, 800)

    for k in range(num_circles + 1):
        circle = rg.Circle(center_location, radii_multiplier * k)
        circle.attach_to(window)
        window.render(0.05)  # Pauses for 0.05 seconds after rendering.
